...
__forceinline__ __device__ double cuda_rsqrt(const double &value)
{
    return rsqrt(value);
}

__forceinline__ __device__ float cuda_rsqrt(const float &value)
{
    return rsqrtf(value);
}
...
{ // number of time ticks (n)
    ...
    for (index_t offset = num_features / 2; offset > 0; offset /= 2)
        x += __shfl_down_sync(0xFFFFFFFF,x, offset, num_features);

    // 2b. perform the first broadcast
    value_t mu = __shfl_sync(0xFFFFFFFF,x, 0) / num_features;
    ...
    for (index_t offset = num_features / 2; offset > 0; offset /= 2)
        y += __shfl_down_sync(0xFFFFFFFF,y, offset, num_features);

    // 3b. perform the second broadcast
    value_t sigma = __shfl_sync(0xFFFFFFFF,y, 0) / (num_features - 1);
...
int main()
{
    ...
    // malloc memory
    cudaMallocHost(&data, sizeof(value_t) * num_entries * num_features);
    CUERR
    cudaMalloc(&Data, sizeof(value_t) * num_entries * num_features);
    CUERR
    ...
    cudaMemcpy(Data, data, sizeof(value_t) * num_entries * num_features, H2D);
    CUERR
    TIMERSTOP(copy_data_to_device)

    TIMERSTART(z_norm)
    znorm_kernel<<<num_entries, 32>>>(Data, num_entries, num_features);
    CUERR
    TIMERSTOP(z_norm)

    TIMERSTART(copy_data_to_host)
    cudaMemcpy(data, Data, sizeof(value_t) * num_entries * num_features, D2H);
    CUERR
